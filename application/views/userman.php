                ...
                <table id="table_id" class="table table-striped table-hover" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo base_url('index.php/uman/action?reset='); echo $this->safe->convert($post->username,$this->session->userdata("user_name")); ?>" data-toggle="tooltip" title="Reset Sandi manjadi '<?php echo $post->username; ?>'" onclick="return confirm('Reset Sandi <?php echo $post->nama_user; ?> menjadi <?php echo $post->username; ?>?')"><button class="btn btn-success btn-xs"><i class="fa fa-undo"></i> Reset Sandi</button></a>
                                <a href="<?php echo base_url('index.php/uman/action?admin='); echo $this->safe->convert($post->username,$this->session->userdata("user_name")); ?>" data-toggle="tooltip" title="Jadikan Admin: '<?php echo $post->username; ?>'" onclick="return confirm('Jadikan <?php echo $post->nama_user; ?> sebagai Admin?')"><button class="btn btn-warning btn-xs"><i class="fa fa-user-circle"></i> Jadi Tim Arsip</button></a>
                                <a href="<?php echo base_url('index.php/uman/action?delete='); echo $this->safe->convert($post->username,$this->session->userdata("user_name")); ?>" data-toggle="tooltip" title="Hapus akun '<?php echo $post->username; ?>'" onclick="return confirm('Hapus akun <?php echo $post->nama_user; ?>?')"><button class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Hapus Akun</button></a>
                            </td>
                        </tr>
                        <?php }}else{echo "<tr><td colspan='5' align='center'> <b><i>*NO USER TO SHOW*</i></b> </td></tr>";}; ?>
                    </tbody>
                </table>
               </div>
			  </div>
